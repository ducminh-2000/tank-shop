...
var FireLoopRef = /** @class */ (function () {
    ...
    **/
    function FireLoopRef(model, socket, parent, relationship) {
        ...
        // Disposable Events
        this.disposable = {};
        this.socket.emit("Subscribe.".concat(!parent ? model.getModelName() : parent.model.getModelName()), { id: this.id, scope: model.getModelName(), relationship: relationship });
        return this;
    }
    /**
    * @method dispose
    ...
    **/
    FireLoopRef.prototype.onRemote = function (method) {
        ...
        if (!this.relationship) {
            event = "".concat(this.model.getModelName(), ".").concat(event);
        }
        else {
            event = "".concat(this.parent.model.getModelName(), ".").concat(this.relationship, ".").concat(event);
        }
    ...
    **/
    FireLoopRef.prototype.child = function (relationship) {
        ...
        if (!settings) {
            throw new Error("Invalid model relationship ".concat(this.model.getModelName(), " <-> ").concat(relationship, ", verify your model settings."));
        }
        // Verify if the relationship model is public
        if (settings.model === '') {
            throw new Error("Relationship model is private, cam't use ".concat(relationship, " unless you set your model as public."));
        }
    ...
    **/
    FireLoopRef.prototype.pull = function (event, request) {
        var sbj = new Subject();
        var that = this;
        var nowEvent = "".concat(event, ".pull.requested.").concat(this.id);
        this.socket.emit("".concat(event, ".pull.request.").concat(this.id), request);
    ...
    **/
    FireLoopRef.prototype.broadcasts = function (event, request) {
        var sbj = new Subject();
        var channels = {
            announce: "".concat(event, ".broadcast.announce.").concat(this.id),
            broadcast: "".concat(event, ".broadcast.").concat(this.id)
        };
        var that = this;
        // Announces Handler
        this.disposable[channels.announce] = function (res) {
            that.socket.emit("".concat(event, ".broadcast.request.").concat(that.id), request);
        };
        // Broadcasts Handler
    ...
    **/
    FireLoopRef.prototype.operation = function (event, data) {
        if (!this.relationship) {
            event = "".concat(this.model.getModelName(), ".").concat(event, ".").concat(this.id);
        }
        else {
            event = "".concat(this.parent.model.getModelName(), ".").concat(this.relationship, ".").concat(event, ".").concat(this.id);
        }
        ...
        var resultEvent = '';
        if (!this.relationship) {
            resultEvent = "".concat(this.model.getModelName(), ".value.result.").concat(this.id);
        }
        else {
            resultEvent = "".concat(this.parent.model.getModelName(), ".").concat(this.relationship, ".value.result.").concat(this.id);
        }
